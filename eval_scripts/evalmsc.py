...
def read_write_gold(file_in, file_out, is_format2016=False, merge_same_keys=False):
    """ Read gold file and write the gold answer only in the same order"""
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description='SIGMORPHON 2016 Shared Task Evaluation with CLUZH extensions')
    ...
    parser.add_argument("--store_gold", default=False,  action='store_true') # store the gold solutions (only one column at the same place as the guesses)
    args = parser.parse_args()

    # store key info of golden for later evaluation
    if args.store_gold:
        if 'test' in args.guesses:
            outfile = os.path.join(os.path.dirname(args.guesses),'test.solution.golden')
        if 'dev' in args.guesses:
            outfile = os.path.join(os.path.dirname(args.guesses),'dev.solution.golden')
        read_write_gold(args.golden, outfile, is_format2016=args.format2016, merge_same_keys=args.merge_same_keys)
        exit(0)
